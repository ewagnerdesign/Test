  ...
  module Helpers
    module FormHelper
      ...
      def script_tags(js_code = '')
         ...
         # -- ( js_code.blank? ? '' : "" ) ++ ( js_code.blank? ? '' : content_tag("script",js_code) )
         # see http://github.com/porras/livevalidation/issues#issue/3 
         ( js_code.blank? ? '' : content_tag("script",js_code) )
      end
    end
  end
end
